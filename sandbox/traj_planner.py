...
class WaypointGenerator:
    ...
    def get_waypoints(self, vehicle) -> List[Waypoint3D]:
        ...
        for distance in self.distances:
            # Get waypoint at this distance along the road
            next_wps = tracking_wp.next(distance)
    ...
    def get_waypoints_smooth(self, vehicle, num_points: int = 10, 
                             max_distance: float = 20.0) -> List[Waypoint3D]:
        """
        Get evenly spaced waypoints for smoother trajectory.
        """
        vehicle_location = vehicle.get_transform().location
        current_wp = self.map.get_waypoint(vehicle_location)
        ...
        # Get many waypoints and interpolate
        step = max_distance / (num_points * 2)
        raw_waypoints = []
        ...
        while traveled < max_distance and wp is not None:
            raw_waypoints.append(Waypoint3D(
                x=wp.transform.location.x,
                y=wp.transform.location.y,
                z=wp.transform.location.z
            ))
            ...
            wp = next_wps[0] if next_wps else None
            traveled += step
        
        if len(raw_waypoints) < num_points:
            return raw_waypoints
        
        # Resample to exact number of points
        indices = np.linspace(0, len(raw_waypoints) - 1, num_points).astype(int)
        return [raw_waypoints[i] for i in indices]
...
class TrajectoryEncoder:
    ...
    def _get_camera_world_transform(self, vehicle_transform):
        ...
        # Camera rotation (vehicle + camera offset)
        cam_rotation = {
            'pitch': v_rot.pitch + self.config.pitch,
            'yaw': v_rot.yaw + self.config.yaw,
            'roll': v_rot.roll + self.config.roll
        }
        
        return cam_location, cam_rotation
    
    def _build_rotation_matrix(self, rotation: dict) -> np.ndarray:
        """Build rotation matrix from camera to world, then invert."""
        # CARLA rotation
        R_carla = rotation_matrix_from_euler(
            rotation['pitch'], rotation['yaw'], rotation['roll']
        )
        
        # Combined: CARLA world → Camera frame
        # First rotate from world to CARLA camera orientation
        # Then apply CARLA-to-OpenCV transform
        R_world_to_cam = self._carla_to_cv @ R_carla.T
...
class TrajectoryDecoder:
    ...
    def _build_rotation_matrix(self, rotation: dict) -> np.ndarray:
        ...
        # Encoder does: R_world_to_cam = carla_to_cv @ R_carla.T
        # So the inverse is: R_cam_to_world = R_carla @ carla_to_cv.T
        # This ensures proper roundtrip: encode(decode(x)) ≈ x
        R_cam_to_world = R_carla @ self._carla_to_cv.T
