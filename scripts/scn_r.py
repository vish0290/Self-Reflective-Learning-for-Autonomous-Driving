...
class LaneFollowingController:
    ...
    def _compute_speed_control(self, current_speed_kmh: float, steer_magnitude: float):
        ...
        if speed_error > 0:
            ...
            throttle = min(0.7, max(0.45, speed_error / 15.0))
            brake = 0.0
        else:
            # Decelerate
            throttle = 0.0
...
class RouteBrowser:
    ...
    def __init__(self, width: int = 1280, height: int = 720):
        ...
        self.custom_routes = []  # List to store user-created routes
        ...
        self.custom_route_mode = False
        self.selected_start_spawn = None
        self.selected_end_spawn = None
        self.spawn_point_positions = {}  # Cache spawn point positions for click detection

        # Pending route tracking (for save/cancel logic)
        self._pending_route_added = False
        self._current_driving_route = None  # Track the route being test-driven

        # Filename input state
        self.filename_input_mode = False
        self.filename_input_text = ""
    ...
    def connect(self, host='localhost', port=2000, town='Town01'):
        """Connect to CARLA."""
    ...
    def save_single_route(self, route: 'ScenarioRoute', filename: str):
        """
        Save a single route to a file.

        Args:
            route: The route to save
            filename: Filename for the route
        ...
        # Create a dict with just this one route
        single_route_scenarios = {"custom": [route]}

        filepath = self.checkpoint_manager.save_scenarios(single_route_scenarios, filename)
        print(f"✓ Saved route to: {filepath}")
        return filepath

    def save_custom_routes(self, filename: str = None):
        """
        Save only the custom routes created by user.

        Args:
            filename: Optional filename. If None, generates one automatically.

        Returns:
            Path to saved file
        """
        if not self.custom_routes:
            print("No custom routes to save!")
            return None

        # Create a dict with only custom routes
        custom_scenarios = {"custom": self.custom_routes}

        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            # Sanitize map name to remove slashes and special characters
            safe_map_name = self.carla_map.name.replace('/', '_').replace('\\', '_').split('/')[-1]
            filename = f"custom_routes_{safe_map_name}_{timestamp}.json"

        # Ensure .json extension
        if not filename.endswith('.json'):
            filename = filename + '.json'

        filepath = self.checkpoint_manager.save_scenarios(custom_scenarios, filename)
        print(f"Saved {len(self.custom_routes)} custom route(s) to: {filepath}")
        return filepath

    def load_custom_routes(self, filename: str):
        """
        Load custom routes from a file.
        
        Args:
            filename: Path to the custom routes file
        """
        loaded_scenarios = self.checkpoint_manager.load_scenarios(filename)
        
        if "custom" in loaded_scenarios:
            self.custom_routes = loaded_scenarios["custom"]
            
            # Also add to scenarios for browser display
            if "custom" not in self.scenarios:
                self.scenarios["custom"] = []
            self.scenarios["custom"].extend(self.custom_routes)
            
            print(f"Loaded {len(self.custom_routes)} custom route(s)")
        else:
            print("No custom routes found in file")
    ...
    def enter_custom_route_mode(self):
        """Enter custom route creation mode."""
        # Clear any pending unsaved route
        self._clear_pending_route()

        self.custom_route_mode = True
        self.selected_start_spawn = None
        self.selected_end_spawn = None
        print("=== CUSTOM ROUTE CREATION MODE ===")
        print("Click on spawn points to select start and end locations")
        print("Press ESC to cancel")

    def exit_custom_route_mode(self):
        """Exit custom route creation mode."""
        # Clear any pending unsaved route
        self._clear_pending_route()
        ...
        self.selected_start_spawn = None
        self.selected_end_spawn = None
        print("Custom route mode cancelled")

    def _clear_pending_route(self):
        """Clear any pending route that hasn't been saved."""
        # Remove the last unsaved route from custom_routes if it exists
        if hasattr(self, '_pending_route_added') and self._pending_route_added:
            if self.custom_routes and len(self.custom_routes) > 0:
                # Remove last route that was added but not saved
                removed = self.custom_routes.pop()
                print(f"Cleared pending route: {removed.id[:40]}...")

                # Also remove from scenarios if it was added there
                if "custom" in self.scenarios and removed in self.scenarios["custom"]:
                    self.scenarios["custom"].remove(removed)

            self._pending_route_added = False

    def get_spawn_point_at_position(self, mouse_x: int, mouse_y: int) -> Optional[int]:
        """
        Get spawn point index at mouse position.
        Returns the index if a spawn point is clicked, None otherwise.
        """
        click_radius = 10

        for spawn_idx, (px, py) in self.spawn_point_positions.items():
    ...
    def handle_spawn_point_click(self, spawn_idx: int):
        """Handle clicking on a spawn point."""
        if self.selected_start_spawn is None:
            self.selected_start_spawn = spawn_idx
            print(f"Start point selected: Spawn #{spawn_idx}")
        elif self.selected_end_spawn is None:
            if spawn_idx == self.selected_start_spawn:
                print("End point must be different from start point!")
                return

            self.selected_end_spawn = spawn_idx
            print(f"End point selected: Spawn #{spawn_idx}")
            print("Creating route...")

            # Create and drive the route
            route = self.create_route_between_spawn_points(self.selected_start_spawn, self.selected_end_spawn)

            if route:
                # Add to custom routes collection (temporarily, until saved)
                self.custom_routes.append(route)

                # Also add to scenarios under "custom" category for browser display
                if "custom" not in self.scenarios:
                    self.scenarios["custom"] = []
                self.scenarios["custom"].append(route)

                # Mark that we have a pending route
                self._pending_route_added = True
                self._current_driving_route = route

                if self.route_driver.start_drive(route):
                    self.is_test_driving = True
                    self.custom_route_mode = False
                    print(f"Starting custom route: {route.total_distance:.0f}m")
                    print("TIP: Press 'S' to save this route (you don't need to wait for completion!)")
            else:
                print("Failed to create route - try different spawn points")
                self.selected_start_spawn = None
                self.selected_end_spawn = None
    ...
    def render(self):
        ...
        elif self.custom_route_mode:
            self._render_spawn_point_selector()
        else:
            self._render_browser_view()

        # Render filename input overlay if active
        if self.filename_input_mode:
            self._render_filename_input()
    ...
    def _render_browser_view(self):
        ...
        # Instructions
        instructions = [
            "1-6: Select type | 7: Custom routes | UP/DOWN: Navigate | T: Test drive",
            "C: Create custom route | S: Save custom | L: Load | F: Find | Q: Quit"
        ]
    ...
    def _render_test_drive_view(self):
        ...
        if self.route_driver and self.route_driver.current_route:
            ...
            y += 50
            if self._pending_route_added:
                save_text = "⚠ UNSAVED ROUTE"
                save_color = Colors.YELLOW
            else:
                save_text = "✓ Route Saved"
                save_color = Colors.GREEN

            surf = self.font.render(save_text, True, save_color)
            self.display.blit(surf, (panel_x, y))
    ...
    def _render_route_type_buttons(self):
        ...
        route_types = [
            ...
            ("custom", "7. Custom Routes"),
        ]

        for rt, label in route_types:
            rect = pygame.Rect(x, y, btn_width, btn_height)
            
            # Get color for this route type
            if rt == "custom":
                color = Colors.WHITE
                rt_value = "custom"
            else:
            ...
            # Highlight selected
            if self.selected_type == rt_value:
            ...
            # Count
            count = len(self.scenarios.get(rt_value, []))
            text = f"{label} ({count})"
    ...
    def _render_filename_input(self):
        """Render the filename input overlay."""
        ...
        pygame.draw.rect(self.display, Colors.DARK_GRAY, (box_x, box_y, box_width, box_height))
        pygame.draw.rect(self.display, Colors.CYAN, (box_x, box_y, box_width, box_height), 3)

        # Title
        title = self.font_large.render("Save Custom Routes", True, Colors.CYAN)
        title_rect = title.get_rect(center=(self.width // 2, box_y + 30))
        self.display.blit(title, title_rect)

        # Prompt
        prompt = self.font.render("Enter filename (without .json):", True, Colors.WHITE)
        prompt_rect = prompt.get_rect(center=(self.width // 2, box_y + 70))
        ...
        input_box_y = box_y + 100

        pygame.draw.rect(self.display, Colors.BLACK, (input_box_x, input_box_y, input_box_width, input_box_height))
        pygame.draw.rect(self.display, Colors.WHITE, (input_box_x, input_box_y, input_box_width, input_box_height), 2)

        # Display current text
        text_surface = self.font.render(self.filename_input_text + "|", True, Colors.WHITE)
        self.display.blit(text_surface, (input_box_x + 10, input_box_y + 10))
        ...
        for i, instr in enumerate(instructions):
            surf = self.font.render(instr, True, Colors.GRAY)
            surf_rect = surf.get_rect(center=(self.width // 2, box_y + 155 + i * 20))
    ...
    def _render_spawn_point_selector(self):
        """Render the spawn point selection interface for creating custom routes."""
        # Title
        title = self.font_large.render("CUSTOM ROUTE CREATION", True, Colors.CYAN)
        self.display.blit(title, (20, 20))

        # Instructions
        instruction_y = 70
        instructions = [
            "Click on spawn points to select start and end locations",
            "ESC: Cancel",
        ]

        for instr in instructions:
            surf = self.font.render(instr, True, Colors.GRAY)
            self.display.blit(surf, (20, instruction_y))
            instruction_y += 25

        # Panel showing selected points
        panel_x = 20
        panel_y = 140
        panel_width = 400
        panel_height = 80

        pygame.draw.rect(self.display, Colors.DARK_GRAY, (panel_x, panel_y, panel_width, panel_height))
        pygame.draw.rect(self.display, Colors.CYAN, (panel_x, panel_y, panel_width, panel_height), 2)

        info_y = panel_y + 15
        if self.selected_start_spawn is not None:
            text = f"✓ Start Point: Spawn #{self.selected_start_spawn}"
            color = Colors.GREEN
        else:
            text = "○ Start Point: (click to select)"
            color = Colors.YELLOW

        surf = self.font.render(text, True, color)
        self.display.blit(surf, (panel_x + 15, info_y))

        info_y += 35
        if self.selected_end_spawn is not None:
            text = f"✓ End Point: Spawn #{self.selected_end_spawn}"
            color = Colors.GREEN
        else:
            text = "○ End Point: (click to select)"
            color = Colors.YELLOW

        surf = self.font.render(text, True, color)
        self.display.blit(surf, (panel_x + 15, info_y))

        # Map area with spawn points
        map_x = 480
        map_y = 20
        map_width = 760
        map_height = 660

        # Calculate bounds for all spawn points
        if self.spawn_points:
            ...
            def world_to_map_local(wx, wy):
                px = map_x + (wx - x_min) / x_range * (map_width - 20)
                py = map_y + (wy - y_min) / y_range * (map_height - 20)
                return px, py

            # Draw map background
            map_rect = pygame.Rect(map_x, map_y, map_width, map_height)
            pygame.draw.rect(self.display, Colors.BLACK, map_rect)
            pygame.draw.rect(self.display, Colors.GRAY, map_rect, 2)

            # Draw spawn points
            for idx, spawn in enumerate(self.spawn_points):
                px, py = world_to_map_local(spawn.location.x, spawn.location.y)
                self.spawn_point_positions[idx] = (px, py)

                # Color based on selection state
                if idx == self.selected_start_spawn:
                    color = Colors.GREEN
                    size = 8
                elif idx == self.selected_end_spawn:
                    color = Colors.RED
                    size = 8
                else:
                    color = Colors.CYAN
                ...
                pygame.draw.circle(self.display, color, (int(px), int(py)), size)
                pygame.draw.circle(self.display, Colors.WHITE, (int(px), int(py)), size, 1)

            # Draw line between selected points
            if self.selected_start_spawn is not None and self.selected_end_spawn is not None:
                start_pos = self.spawn_point_positions[self.selected_start_spawn]
                end_pos = self.spawn_point_positions[self.selected_end_spawn]
                pygame.draw.line(self.display, Colors.YELLOW, start_pos, end_pos, 2)

            # Info panel
            info_x = 20
            info_y = 250
            info = [
                f"Total Spawn Points: {len(self.spawn_points)}",
                "",
                "Color Guide:",
                "  ● Green: Start point (selected)",
                "  ● Red: End point (selected)",
                "  ● Cyan: Available spawn point",
            ]

            for text in info:
                color = Colors.WHITE if text and not text.startswith("●") else Colors.GRAY
                surf = self.font.render(text, True, color)
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            elif event.type == pygame.MOUSEBUTTONDOWN and self.custom_route_mode:
                if event.button == 1:  # Left click
                    spawn_idx = self.get_spawn_point_at_position(event.pos[0], event.pos[1])
                    if spawn_idx is not None:
                        self.handle_spawn_point_click(spawn_idx)

    def _handle_keydown(self, event):
        """Handle key presses."""
        # Handle filename input mode
        if self.filename_input_mode:
            self._handle_filename_input_keys(event)
            return
        ...
        # Handle custom route mode keys
        if self.custom_route_mode:
            if event.key == pygame.K_ESCAPE:
                self.exit_custom_route_mode()
            return
        ...
        # Save scenarios (all scenarios and custom routes)
        elif event.key == pygame.K_s:
            if self.custom_routes:
                # Enter filename input mode for custom routes
                self.filename_input_mode = True
                self.filename_input_text = ""
                print("Enter filename for custom routes...")
            elif self.scenarios:
        ...
        # Load scenarios
            ...
            # Clear any pending unsaved routes before loading
            self._clear_pending_route()
            ...
            if files:
                ...
                # Try loading as custom routes first, otherwise as scenarios
                if "custom_routes" in latest:
                    self.load_custom_routes(latest)
                    print(f"Loaded custom routes from: {latest}")
                else:
                    self.load_scenarios(latest)
                    print(f"Loaded scenarios from: {latest}")
            else:
        ...
        # Create custom route between spawn points
        elif event.key == pygame.K_c:
            self.enter_custom_route_mode()

    def _handle_filename_input_keys(self, event):
        """Handle keys during filename input mode."""
        if event.key == pygame.K_RETURN or event.key == pygame.K_KP_ENTER:
            # Save with entered filename
            self.filename_input_mode = False
            filename = self.filename_input_text.strip() if self.filename_input_text.strip() else None

            # If we're saving during test drive, save only the current route
            if self._current_driving_route:
                if not filename:
                    # Generate default filename
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    filename = f"route_{timestamp}"

                filepath = self.save_single_route(self._current_driving_route, filename)
                if filepath:
                    # Mark route as saved and remove from pending list
                    self._pending_route_added = False
                    # Remove from custom_routes list to prevent duplication
                    if self._current_driving_route in self.custom_routes:
                        self.custom_routes.remove(self._current_driving_route)
                    if "custom" in self.scenarios and self._current_driving_route in self.scenarios["custom"]:
                        self.scenarios["custom"].remove(self._current_driving_route)
                    self._current_driving_route = None
                    print(f"✓ Route saved successfully!")
            else:
                # Fallback to saving all custom routes
                filepath = self.save_custom_routes(filename)
                if filepath:
                    self._pending_route_added = False
                    self._current_driving_route = None
            ...
            self.filename_input_text = self.filename_input_text[:-1]

        else:
            # Add character to filename (only alphanumeric, underscore, dash)
            if event.unicode and (event.unicode.isalnum() or event.unicode in ['_', '-', ' ']):
                self.filename_input_text += event.unicode
    ...
    def _handle_test_drive_keys(self, event):
        ...
        if event.key == pygame.K_ESCAPE:
            ...
            # Save the current route immediately (no need to wait for completion)
            if self._pending_route_added and self._current_driving_route:
                # Enter filename input mode
                self.filename_input_mode = True
                self.filename_input_text = ""
                print("Enter filename to save route (you don't need to wait for completion)...")
            elif not self._pending_route_added:
                print("Route already saved!")
            else:
                print("No route to save!")
            ...
            if self.route_driver and self.route_driver.current_route:
                self.route_driver.start_drive(self.route_driver.current_route)
    ...
    def _stop_test_drive(self):
        ...
        # Warn if there's an unsaved route
        if self._pending_route_added:
            print("⚠ WARNING: You have an unsaved route! It will be discarded.")
